...
- 「OK」ボタン (ボタンの文言は「許可する」でも良いかもしれません)
#### 5. OSの通知許可ダイアログ
- ユーザーが「OK」をタップすると表示される
#### 6.  メイン画面
...
### プッシュ通知
#### 実装方針
Firebase Cloud Messaging (FCM)とExpo Notficationsを組み合わせて使う
#### 通知内容
- **タイトル**：「お天気おかん」
...
- weatherMessages コレクション: 全ユーザーが読み取り可能、書き込みは認証された特定のユーザー・サービスアカウントからのみ許可（Cloud Functionsを想定）
### アプリのバージョニングとデプロイ
